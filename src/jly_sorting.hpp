...
template <typename T>
inline size_t median_of_st_mid_en(std::vector<T>& data, size_t st, size_t en)
{
	size_t mid = (st+en)/2;
	if(data[st] < data[en])
	{
...
//median of 3 numbers
template <typename T>
inline size_t median_of_3(std::vector<T>& data, size_t st)
{
	T* data_ = &data[st];
...
template <typename T>
inline size_t median_of_5(std::vector<T>& data, size_t st)
{
	T* data_ = &data[st];
	T tmp;
...
template <typename T>
size_t median_of_medians(std::vector<T>& data, size_t st, size_t en)
{
	size_t l = en-st+1;
	size_t numof5 = l / 5;
	if(l % 5 != 0)
...
template <typename T>
void insertion_sort(std::vector<T>& data, size_t st, size_t en)
{
	T tmp;
	size_t i, j;
	for(i = st+1; i <= en; i++)
...
template <typename T>
void intro_select(std::vector<T>& data, size_t st, size_t en, size_t k)
{	
	T pivot;
	T tmp;
